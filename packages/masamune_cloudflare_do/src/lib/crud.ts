import { DoClient, encode, decode, quote } from "./client";
import { HttpError } from "./http_error";
import type { SchemaTable, CrudRequest as Request, DoCrudMethod } from "./types";
import { validateVectors } from "./vector";
/** 同一カラムの複数条件もANDで保持する。値や識別子を文字列連結しない。 */
export function directWhere(client: DoClient, schema: SchemaTable, request: Request): { sql: string; parameters: unknown[] } {
...
export function selectDirectRows(client: DoClient, request: Request, maxScanRows = 1000): Record<string, unknown>[] {
  if(request.nearest || request.bookmark) throw new HttpError(400, "nearest/bookmarkはDOでは未対応です。");
  ...
  const rows = client.execute(`SELECT * FROM ${tableSql(schema)}${where.sql}${order} LIMIT ?`,
    [...where.parameters, Math.min(request.limit ?? max + 1, max + 1)]);
  if(rows.length > max) throw new HttpError(413, "Do scan exceeded maxScanRows.");
  return rows.map(row => decode(row, schema));
}
export function fetchDirectDocumentForRules(client: DoClient, request: Request, maxScanRows = 1000): Record<string, unknown> | null {
  return (selectDirectRows(client, { ...request, limit: 1 }, maxScanRows))[0] ?? null;
}

/** 一つの変更はRETURNINGを含む一文で実行する。batchでも同じ文を使う。 */
export function mutation(client: DoClient, method: DoCrudMethod, request: Request): { sql: string; parameters: unknown[]; schema: SchemaTable } {
  if(request.nearest || request.bookmark) throw new HttpError(400, "nearest/bookmarkはDOでは未対応です。");
  const schema = client.table(request.database, request.table);
...
export function executeCrud(client: DoClient, method: DoCrudMethod, request: Request, max = 1000): unknown {
  if(method === "GET") {
    if(!request.count) return selectDirectRows(client, request, max);
    if(request.nearest || request.bookmark) throw new HttpError(400, "nearest/bookmarkはDOでは未対応です。");
  const schema = client.table(request.database, request.table);
    const where = directWhere(client, schema, request);
    const rows = client.execute(`SELECT COUNT(*) AS count FROM ${quote(schema.table)}${where.sql}`, where.parameters);
    return rows[0].count;
  }
  const statement = mutation(client, method, request);
  const rows = client.execute(statement.sql, statement.parameters);
