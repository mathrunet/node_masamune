...
export type DoServerErrorLogDetails = Readonly<{
  operation?: string;
  phase?: string;
  method?: string;
  database?: string;
...
export async function parseCrudRequest(
  context: Context,
): Promise<Required<Pick<DoRequestBody, "database" | "table">> & DoRequestBody> {
  ...
  const body = method === "GET"
    ...
    : await parseJsonBody<DoRequestBody>(context);
...
export function logServerError(
  error: unknown,
  status: number,
  details: DoServerErrorLogDetails = {},
): void {
  if(status < 500) {
    return;
  }
  console.error("Do request failed", {
    status,
    ...details,
...
function parseGetRequest(context: Context): DoRequestBody {
  const query = context.req.query();
  const where = query.where ? parseJsonString<DoWhereCondition[]>(query.where, "where") : undefined;
  const orderBy = query.orderBy ? parseJsonString<DoOrderCondition[]>(query.orderBy, "orderBy") : undefined;
...
async function parseJsonBody<T>(context: Context): Promise<T> {
  try {
    ...
    if(context.req.method === "DELETE" && !text.trim()) return {} as T;
...
function validateWhere(where: DoWhereCondition[]): DoWhereCondition[] {
...
function validateOrderBy(orderBy: DoOrderCondition[]): DoOrderCondition[] {
