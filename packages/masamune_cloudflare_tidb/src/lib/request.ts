...
function parseGetRequest(context: Context): TidbRequestBody {
  const query = context.req.query();
  const where = query.where ? parseJsonString<TidbWhereCondition[]>(query.where, "where") : undefined;
...
function validateOrderBy(orderBy: TidbOrderCondition[]): TidbOrderCondition[] {
