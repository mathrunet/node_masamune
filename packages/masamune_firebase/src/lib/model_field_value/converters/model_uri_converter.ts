...
export class FirestoreModelUriConverter extends FirestoreModelFieldValueConverter {
  ...
  convertFrom(
    ...
    } else if (utils.isDynamicMap(value)) {
      const targetKey = `#${key}`;
  ...
  convertTo(
    ...
    if (utils.isDynamicMap(value) && value["@type"] !== undefined) {
      ...
      const list = value.filter(e => utils.isDynamicMap(e));
      ...
      const map = Object.entries(value).filter(([_, v]) => utils.isDynamicMap(v));
