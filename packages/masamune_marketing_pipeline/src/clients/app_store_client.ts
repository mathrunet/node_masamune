 ...
 * - App information
 * - Ratings and reviews
 * - Downloads and sales (via Sales and Trends reports)
 *
...
import { DateRange, AppStoreData, RatingDistribution, Review } from "../models/marketing_data";
...
export class AppStoreClient {
    ...
    private async apiRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
        ...
        if (!response.ok) {
            ...
            throw new MarketingPipelineError(
                `App Store Connect API error: ${response.status} ${response.statusText} - ${errorBody}`,
                response.status === 401 ? ErrorCategory.AUTHENTICATION : ErrorCategory.INTERNAL,
                response.status >= 500
            );
        }
     ...
     */
    async collectAllData(dateRange: DateRange): Promise<AppStoreData> {
        ...
        return {
            appId: this.appId,
            dateRange: dateRange,
            // Download and sales data would require Sales and Trends API
            // which needs additional vendor setup
            totalDownloads: undefined,
            redownloads: undefined,
            updates: undefined,
            proceeds: undefined,
            proceedsCurrency: undefined,
